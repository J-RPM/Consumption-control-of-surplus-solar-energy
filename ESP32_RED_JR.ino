...
long clkTimeR = 0;      // RED Check Timer
...
bool Manual_FORCED = false;
bool Status_RED = false;
bool Status_BACK = false;
bool RED1 = true;
int Min_RED = 1;        // Minutes between two automatic actions to OFF
...
// RED
#define IN_RED  14       //AC: 2   
#define OUT_CONTROL  2   //AC: 14 
String msgRED = "GRID";
...
void setup() {
  ...
  // RED
  pinMode(OUT_CONTROL, OUTPUT);     // Output CONTROL (ON/OFF)
  digitalWrite(OUT_CONTROL, LOW);   // CONTROL = OFF
  Status_RED = false;
  pinMode(IN_RED, INPUT_PULLUP);    // Status Input Pull-Up (Status RED: ON/OFF)
  readConfig();
  load_RED();
  initInterval();
  ...
  // Debug first message 
  String stringMsg = "ESP32_RED_JR " + HWversion + " - RTC: ";
  if (T_Zone2 == false) {
...
void readConfig(){
  ...
  PRINT("\nMinutes RED_ON > ",Min_ON);    // Minutes between two RED automatic actions to ON
  PRINT("\nMinutes RED_OFF > ",Min_RED);  // Minutes between two RED automatic actions to OFF
...
void matrix_RED() {
  ...
  if (msgRED == "G-AON") {
    nTag = " A-ON";  
  }else if (msgRED == "G-AOF") {
    nTag = "A-OFF";  
  }else if (msgRED == "G-MON") {
    nTag = " M-ON";  
  }else if (msgRED == "G-FON") {
    nTag = "M-FON";  
  }else if (msgRED == "STOP") {
    nTag = " " +  msgRED;  
  }else {
  ...
  // Show RED action on matrix dispaly
  lc.setChar(0,7,'R',false);
...
void append_webpage_header() {
  ...
  webpage += "<html><head><title>NTP Time & Control RED</title>";
  webpage += "</head>";
  ...
  webpage += "<div id=\"mark\">";
  webpage += "<div id=\"header\"><h1 class=\"animate__animated animate__flash\">NTP Time & Control RED " + HWversion + "</h1>";
}
//////////////////////////////////////////////////////////////////////////////
void button_Home() {
...
void NTP_Clock_home_page() {
  ...
  if (display_inverter==false) webpage += "Grid"; else webpage += msgRED;
  ...
  if (display_inverter==true) {
    ...
    if (RED1==true) webpage += "1"; else webpage += "2"; 
    webpage += " kW</button></a>";

    // RED: ON/OFF
    webpage += "<a href=\"\\RED_C\"><type=\"button\" class=\"button\">RED = ";
    if (Status_RED==true){
      if (Manual_FORCED==true) webpage += " F-";
      webpage += "ON";
    }else {
      webpage += "OFF"; 
    }
...
void _interval_1() {
  Min_ON = 1; 
  PRINTS("\n-> Delay RED_ON > 1 minute");
  _save_interval();    
}
/////////////////////////////////////////////////////////////////
void _interval_3() {
  Min_ON = 3; 
  PRINTS("\n-> Delay RED_ON > 3 minutes");
  _save_interval();    
}
/////////////////////////////////////////////////////////////////
void _interval_5() {
  Min_ON = 5; 
  PRINTS("\n-> Delay RED_ON > 5 minutes");
  _save_interval();    
}
/////////////////////////////////////////////////////////////////
void _interval_10() {
  Min_ON = 10; 
  PRINTS("\n-> Delay RED_ON > 10 minutes");
  _save_interval();    
}
/////////////////////////////////////////////////////////////////
void _interval_15() {
...
void testRed(){
  ...
  if((P_Grid < RED_ON && Status_RED == false) || (P_Grid > RED_OFF && Status_RED == true)){
    if (Sampling < 250) Sampling = Sampling + 1;       
  }else{
    Sampling = 0;
  }

  if(millis()-clkTimeR > 60000){
    clkTimeR = millis();
    Cont_Min = Cont_Min - 1;

    // Test AUTO RED ON/OFF
    if  ((msgRED == "G-AOF" || msgRED == "G-AON") && Cont_Min < 1){
      if(Sampling > 20){
        Sampling = 0;
        Manual_RED = false;
        changeRED(); // >>> PULSA RED (AUTO) 
      }
    }
   }
  }
 /////////////////////////////////////////////////////////////////
void changeRED() {
  // Change status RED: Start <-> Stop
  if(Status_RED == false){
    digitalWrite(OUT_CONTROL, HIGH);  // CONTROL = ON
    Status_RED = true;
    msgRED="START";
    matrix_RED();
  }else {
...
void setRED() {
  ...
  // Reload, minutes between two automatic RED actions
  if (Status_BACK != Status_RED) {
    Status_BACK = Status_RED;
    initInterval();
  }
 
  if (Manual_RED == true){
    if(Status_RED == true){  // MANUAL & ON
      if (Manual_FORCED == true){
        msgRED="G-FON";
      }else {
        msgRED="G-MON";
      }
